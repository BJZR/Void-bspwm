...
echo -e "${C}Verificando y actualizando repositorios...${N}"
if [ ! -f /etc/xbps.d/00-repository-main.conf ] || ! grep -q "https://repo-default.voidlinux.org/current" /etc/xbps.d/00-repository-main.conf; then
...
xbps-install -y xorg-server xorg-input-drivers xorg-video-drivers \
...
if ! xbps-query lightdm > /dev/null; then
    echo -e "${R}Error: LightDM no se instaló correctamente.${N}"
    exit 1
fi
...
if [ ! -L /var/service/lightdm ]; then
    echo -e "${R}Error: No se pudo habilitar el servicio de LightDM.${N}"
    exit 1
fi
