...
# Instalar primero las dependencias críticas para LightDM
echo -e "${C}Instalando dependencias críticas...${N}"
xbps-install -y xorg-minimal dbus elogind eudev polkit ConsoleKit2

# Instalación de paquetes con nombres corregidos
echo -e "${C}Instalando todos los paquetes necesarios...${N}"
xbps-install -y bspwm sxhkd rofi polybar dunst picom feh yad i3lock \
...
if ! xbps-query lightdm > /dev/null; then
    echo -e "${R}Error: LightDM no se instaló correctamente. Intentando reinstalar...${N}"
    xbps-remove -y lightdm lightdm-gtk-greeter
    xbps-install -Sy lightdm lightdm-gtk-greeter
    if ! xbps-query lightdm > /dev/null; then
        echo -e "${R}Error: No se pudo instalar LightDM. Configurando inicio manual...${N}"
...
for service in dbus elogind polkitd ConsoleKit NetworkManager bluetoothd; do
    if [ -d /etc/sv/$service ]; then
        if [ -L /var/service/$service ]; then
            echo "Servicio $service ya está habilitado. Reiniciando..."
            sv restart $service
        else
        ...
        echo "Servicio $service no encontrado en /etc/sv/"
    fi
done

# LightDM se habilita al final para asegurar que sus dependencias estén listas
if [ -d /etc/sv/lightdm ]; then
    if [ -L /var/service/lightdm ]; then
        echo "Reiniciando servicio LightDM..."
        sv restart lightdm
    else
    ...
    echo "Servicio LightDM no encontrado en /etc/sv/"
fi
...
# Configuración de LightDM con fondo personalizado (simplificada)
...
# Configuración de greeter simplificada
...
if [ -f "$(pwd)/wallpaper.jpg" ] && [ -f "$(pwd)/wallpaper1.jpg" ] && [ -f "$(pwd)/wallpaper2.jpg" ]; then
    cp "$(pwd)/wallpaper"*.jpg "$USER_HOME/"
    chown $USUARIO:$USUARIO "$USER_HOME/wallpaper"*.jpg
else
    echo -e "${Y}ADVERTENCIA: No se encontraron archivos de fondo en $(pwd)${N}"
    echo -e "${Y}Creando fondos predeterminados...${N}"
    convert -size 1920x1080 plasma:blue-navy "$USER_HOME/wallpaper.jpg"
    convert -size 1920x1080 plasma:purple-blue "$USER_HOME/wallpaper1.jpg"
    convert -size 1920x1080 plasma:green-blue "$USER_HOME/wallpaper2.jpg"
    chown $USUARIO:$USUARIO "$USER_HOME/wallpaper"*.jpg
    
    # Actualizar la ruta del fondo en lightdm-greeter
    sed -i "s|background = $SCRIPT_DIR/wallpaper2.jpg|background = $USER_HOME/wallpaper2.jpg|g" /etc/lightdm/lightdm-gtk-greeter.conf
fi
...
#!/bin/sh
if [ "$(sv status bluetoothd | grep -c "^run")" -gt 0 ]; then
    if [ "$(bluetoothctl show | grep -c 'Powered: yes')" -gt 0 ]; then
        echo "%{F#2ecc71}BT ON"

        # Verificar conexión
        if [ "$(bluetoothctl info | grep -c 'Connected: yes')" -gt 0 ]; then
...
# Configurar inicio automático en caso de que falle LightDM
echo -e "${C}Configurando respaldo para inicio automático...${N}"
cat > /etc/profile.d/autologin.sh << EOL
if [ -z "\$DISPLAY" ] && [ "\$(tty)" = "/dev/tty1" ] && [ "\$(whoami)" = "$USUARIO" ]; then
    exec startx
fi
EOL
chmod +x /etc/profile.d/autologin.sh
...
echo -e "  ${C}1. LightDM debería iniciar automáticamente ahora${N}"
echo -e "  ${C}2. Si LightDM falla, se iniciará automáticamente BSPWM al iniciar sesión en tty1${N}"
echo -e "  ${C}3. Puedes reiniciar manualmente LightDM con: ${N}sudo sv restart lightdm"
